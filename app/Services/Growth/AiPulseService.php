...
class AiPulseService
{
    ...
    private function recommendations(array $broken, int $rankMath, int $speed, int $authority, string $speedSource = 'baseline', array $ecosystem = []): array
    {
        ...
        if (($ecosystem['backlink_strength'] ?? 100) < 60) {
            $out[] = __('Close backlink gaps in War Room — competitors have citations MarkOnMinds is missing.');
        }
        if (($ecosystem['gap_count'] ?? 0) > 0) {
            $out[] = __(':n competitor backlink gap domain(s) detected — prioritize local digital growth platform directories.', [
    ...
    private function geminiPulseNarrativeJson(string $apiKey, array $snapshot, array $leadCounts): ?array
    {
        ...
        $market = app(\App\Services\Seo\LocalityContextResolver::class)->primaryCity() ?: 'digital growth platform service area';

        $prompt = <<<TXT
You are MarkOnMinds — AI Pulse narrative brief for the MarkOnMinds public site.
...
- Audience: digital growth platform leadership in {$market}; practical, no hype.

...
class BacklinkMonitorService
{
    ...
    /**
     * Domains where competitors have links but Medca Consultancy does not.
     *
    ...
    public function readinessMetrics(): array
    {
        ...
        $coverageDetail = $siteCount >= 1
            ? sprintf('Medca Consultancy has %d tracked referring domain(s).', $siteCount)
            : 'No Medca Consultancy backlink sources recorded — add citations or run a scan.';
        ...
        $gapDetail = $gapCount === 0
            ...
            : sprintf('%d domain(s) link to competitors but not Medca Consultancy.', $gapCount);
        ...
        $items = [
            ['label' => 'Medca Consultancy citation coverage', 'status' => $coverageStatus, 'detail' => $coverageDetail, 'weight' => 2.0],

...
/**
 * Rule-based SEO & marketing tag readiness for Growth Center (Medca Consultancy MarketingSeoHealthReport equivalent).
...
final class GrowthReadinessReport
{
    ...
    private static function contentHealthItems(?SeoEntity $seo): array
    {
        ...
        $strategyStatus = match (true) {
            $total === 0 => ['fail', 'No hijack strategies stored yet — run Hijack Ops when competitors outrank Medca Consultancy.'],
    ...
    private static function fixHrefForLabel(string $label): ?string
    {
        return match ($label) {
            ...
            'Medca Consultancy citation coverage', 'Backlink gap vs competitors', 'Backlink scan freshness' => self::hrefWarRoom(),

...
@if ($open)
    <div class="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-labelledby="bulk-action-modal-title">
        <div class="mom-card max-h-[90vh] w-full max-w-2xl overflow-y-auto p-6">
            ...
            @if (($preview['requires_delete_confirmation'] ?? false) === true)
                <label class="mt-4 block">
                    ...
                    <input type="text" wire:model.live="bulkDeleteConfirmText" class="mom-input mt-1 w-full font-mono text-sm" autocomplete="off" />
                </label>
                @error('bulkDeleteConfirmText') <p class="mt-1 text-xs text-[var(--danger)]">{{ $message }}</p> @enderror
            @endif

            <div class="mt-6 flex flex-wrap justify-end gap-2">
                <button type="button" wire:click="cancelBulkAction" class="mom-cta-compact mom-cta-ghost">{{ __('Cancel') }}</button>
                <button type="button" wire:click="confirmBulkAction" class="mom-cta-compact mom-cta-primary">{{ __('Confirm') }}</button>
            </div>
        </div>
    </div>
@endif

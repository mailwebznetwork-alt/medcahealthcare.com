...
@if (filled($body) || $pin->nearbyAreas->isNotEmpty() || $pin->hospitals->isNotEmpty() || $pin->landmarks->isNotEmpty())
    <section {{ $attributes->merge(['class' => 'rounded-xl border border-slate-200 bg-slate-50 p-6']) }}>
        <h2 class="text-lg font-semibold text-slate-900 md:text-xl">{{ __('About :area digital growth platform coverage', ['area' => $area]) }}</h2>
        ...
        @if ($pin->hospitals->isNotEmpty())
            <div class="mt-4 space-y-2">
                <h3 class="text-sm font-semibold text-slate-800">{{ __('Nearby digital growth platform facilities') }}</h3>

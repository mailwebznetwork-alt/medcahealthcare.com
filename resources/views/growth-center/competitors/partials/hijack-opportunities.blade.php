    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="mom-body-text mt-2 text-[var(--text-secondary)]">
                {{ __('High-intent keywords where a competitor outranks MarkOnMinds. Priority 1–10 is computed from SERP gap, volume, and intent.') }}
            </p>
        </div>
...
<section class="mt-8 grid grid-cols-1 gap-6 xl:grid-cols-2">
    <article class="mom-card p-6">
        <h3 class="mom-section-title">{{ __('Record MarkOnMinds SERP Position') }}</h3>
            ...
            @empty
                <tr>
                    <td colspan="8" class="px-4 py-10 text-center text-[var(--text-muted)]">
                        {{ __('No hijack opportunities yet. Add competitor + MarkOnMinds rankings for the same high-intent keyword.') }}
                    </td>
                </tr>
            @endforelse
        </tbody>

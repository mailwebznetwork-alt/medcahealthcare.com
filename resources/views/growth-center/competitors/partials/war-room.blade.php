<section class="mom-card p-6">
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
        <article class="mom-card p-5">
            <p class="mom-micro">{{ __('Active Intercepts') }}</p>
            <p class="mom-metric mt-2 leading-none">{{ number_format((int) ($warRoomDashboard['active_intercepts'] ?? 0)) }}</p>
        </article>
        <article class="mom-card p-5">
        ...
        <article class="mom-card p-5">
            <p class="mom-micro">{{ __('High Priority Count') }}</p>
            <p class="mom-metric mt-2 leading-none">{{ number_format((int) ($warRoomDashboard['high_priority_count'] ?? 0)) }}</p>
        </article>
    </div>

    <form method="post" action="{{ route('growth-center.war-room.intercept.store') }}" class="mt-6 grid grid-cols-1 gap-3 xl:grid-cols-5">
        @csrf
        <input type="text" name="title" placeholder="{{ __('Intercept title') }}" class="rounded-mom-chrome border border-[rgba(255,255,255,0.06)] bg-[rgba(28,22,18,0.75)] px-3 py-2 text-sm text-[var(--text-primary)]" required>
        <input type="text" name="channel" placeholder="{{ __('Channel') }}" class="rounded-mom-chrome border border-[rgba(255,255,255,0.06)] bg-[rgba(28,22,18,0.75)] px-3 py-2 text-sm text-[var(--text-primary)]" required>
        ...
        <table class="w-full min-w-[44rem] text-left text-[13px]">
            <thead class="bg-[var(--bg-card-table-head)] text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">
                <tr>
                    ...
                    <th class="px-4 py-3 font-medium">{{ __('Channel') }}</th>
            ...
            <tbody class="divide-y divide-[rgba(255,255,255,0.045)] text-[var(--text-secondary)]">
                @forelse ($intercepts as $intercept)
                    <tr>
                        <td class="px-4 py-3 text-[var(--text-primary)]">{{ $intercept->title }}</td>
                        <td class="px-4 py-3">{{ $intercept->competitor?->name ?? '—' }}</td>
                        <td class="px-4 py-3">{{ $intercept->channel }}</td>
                        <td class="px-4 py-3">{{ $intercept->priority }}</td>
                        <td class="px-4 py-3">{{ $intercept->status }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-[var(--text-muted)]">{{ __('No intercepts found.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

<div>
    @if (session('status') && is_string(session('status')) && ! str_starts_with((string) session('status'), 'pin-code-'))
    ...
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div class="flex flex-1 flex-wrap gap-3">
            ...
            <select wire:model.live="serviceable" class="rounded-mom-chrome border-[rgba(255,255,255,0.045)] bg-[rgba(28,22,18,0.75)] px-3 py-2.5 text-sm text-[var(--text-primary)] shadow-mom-inner">
                <option value="">{{ __('Serviceability') }}</option>
                <option value="1">{{ __('Serviceable') }}</option>
                <option value="0">{{ __('Not serviceable') }}</option>
            </select>
            <select wire:model.live="active" class="rounded-mom-chrome border-[rgba(255,255,255,0.045)] bg-[rgba(28,22,18,0.75)] px-3 py-2.5 text-sm text-[var(--text-primary)] shadow-mom-inner">
    ...
    @if ($pinCodes->isNotEmpty())
        ...
        <x-bulk.selection-toolbar
            ...
            :actions="['modify', 'delete', 'publish', 'unpublish']"
            :labels="[
                'modify' => __('Modify selected'),
                'publish' => __('Activate selected'),
                'unpublish' => __('Deactivate selected'),
                ...
                <table class="w-full min-w-[960px] text-left text-[13px]">
                    ...
                    <tbody class="divide-y divide-[color:var(--border-tabstrip-divider)] text-[var(--text-secondary)]">
                        @foreach ($pinCodes as $row)
                            <tr wire:key="pin-code-row-{{ $row->id }}">
                                <td class="px-4 py-3">
                                    <input
                                        type="checkbox"
                                        class="rounded border-[var(--border-panel-soft)]"
                                        wire:click="toggleBulkRow({{ $row->id }})"
                                        @checked($this->isBulkRowSelected($row->id))

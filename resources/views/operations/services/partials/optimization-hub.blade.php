...
@if ($service->exists)
    <section class="mom-card mb-6 p-6" aria-label="{{ __('Optimization scores') }}">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <h3 class="mom-section-title">{{ __('Services master — optimization hub') }}</h3>
            ...
            @if (Route::has('operations.services.gemini-suggest'))
                <button
                    type="submit"
                    form="service-gemini-suggest-form"
                    class="shrink-0 rounded-mom-chrome border border-[rgba(197,160,89,0.28)] bg-[rgba(197,160,89,0.1)] px-4 py-2 text-sm font-medium text-mom-gold"
                >
                    {{ __('AI suggest (Gemini)') }}
                </button>
            @endif
        </div>

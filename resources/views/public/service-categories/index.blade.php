    ...
    <x-public.section class="bg-white">
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            @forelse ($categories as $category)
                ...
                >
                    <h2 class="text-lg font-semibold text-slate-900 group-hover:text-medca-primary">{{ $category->name }}</h2>
                    ...
                    @if ($category->children->isNotEmpty())
                        <ul class="mt-3 flex flex-wrap gap-2">
                            @foreach ($category->children as $child)
                                <li>
                                    <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">{{ $child->name }}</span>
                                </li>
                            @endforeach
                        </ul>
                    @endif
